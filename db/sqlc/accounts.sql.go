...
const updateFetchAccount = `-- name: UpdateFetchAccount :one
UPDATE accounts
SET balance = $2
WHERE id = $1
RETURNING id, owner, balance, currency, created_at
...
type UpdateFetchAccountParams struct {
	ID      int64 `json:"id"`
	Balance int64 `json:"balance"`
}

func (q *Queries) UpdateFetchAccount(ctx context.Context, arg UpdateFetchAccountParams) (Account, error) {
	row := q.db.QueryRow(ctx, updateFetchAccount, arg.ID, arg.Balance)
	var i Account
	err := row.Scan(
		&i.ID,
		&i.Owner,
